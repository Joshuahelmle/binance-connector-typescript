...
export interface MarketMethods {
  ...
  exchangeInformation(
    options?: exchangeInformationOptions
  ...
  recentTradesList(
    symbol: string,
    options?: recentTradesListOptions
  ): Promise<recentTradesListResponse[]>;
  oldTradeLookup(
    symbol: string,
    options?: oldTradeLookupOptions
  ): Promise<oldTradeLookupResponse[]>;
  compressedAggregateTradesList(
    symbol: string,
    options?: compressedAggregateTradesListOptions
  ): Promise<compressedAggregateTradesListResponse[]>;
  klineCandlestickData(
    symbol: string,
    interval: Interval,
    options?: klineCandlestickDataOptions
  ): Promise<klineCandlestickDataResponse[]>;
  uiklines(
    symbol: string,
    interval: Interval,
    options?: uiklinesOptions
  ): Promise<uiklinesResponse[]>;
  currentAveragePrice(symbol: string): Promise<currentAveragePriceResponse>;
  ticker24hr(
    options?: ticker24hrOptions
  ): Promise<ticker24hrResponse | ticker24hrResponse[]>;
  symbolPriceTicker(
    options?: symbolPriceTickerOptions
  ): Promise<symbolPriceTickerResponse | symbolPriceTickerResponse[]>;
  symbolOrderBookTicker(
    options?: symbolOrderBookTickerOptions
